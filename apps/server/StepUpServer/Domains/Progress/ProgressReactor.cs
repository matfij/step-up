...
public class ProgressReactor(IProgressRepository repository, IEventPublisher publisher) :
    ...
    IEventHandler<ActivityCreatedEvent>
{
    ...
    public async Task HandleAsync(UserUpdatedEvent userEvent)
    {
        var progress =
            await _repository.GetByUserId(userEvent.UserId)
            ?? throw new ApiException("errors.progressNotFound");

        progress.Username = userEvent.Username;
        progress.AvatarUri = userEvent.AvatarUri;
