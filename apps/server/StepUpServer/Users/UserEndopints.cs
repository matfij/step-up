...
public static class UserEndpints
{
    ...
    {
        app.MapPost("/users/register/start", async (StartRegisterStartRequest request, IUserService userService) =>
        {
        ...
        app.MapPost("/users/register/complete", async (CompleteRegisterRequest request, IUserService userService) =>
        {
            var user = await userService.CompleteRegister(request.Email, request.AuthCode);
            return Results.Ok(new { user.Id, user.Email, user.Username, user.ApiToken });
        });
    }
}
